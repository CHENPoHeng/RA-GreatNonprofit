...
class liwcExtractor():
    ...
    def getCategoryIndeces(self):
        ...
        indeces += ['wc (Word Count)', 'sixltr (Six Letter Words)','dic (In Dictionary)'] # These last two are not built yet.
        return indeces
    ...
    def extractFromDoc(self, document):
        ...
        #print tokens
        features = [0] * (len(self.categories) + 3) 
                            # -3 = wc
                            # -2 = sixltr
                            # -1 = dic
                            # -------------------------
                            # 66 = wc, total word count
                            # 67 = sixltr, six letter words
                            # 68 = dic, words found in LIWC dictionary
                            # 70 = punc, punctuation // not useing
                            # 71 = emoticon  // not using
        features[-3] = len(tokens)
