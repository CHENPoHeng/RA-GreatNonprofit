...
# For the rating distribution violin graph: actually maybe we should make 
# these bar plots given that there are only 5 discrete values. Create one 
# in linear and one in log-scale (looks like most are 5 stars so it is hard 
# to tell the difference between #2star vs. #3star ratings)
d$rating = as.factor(d$rating)
ggplot(d, aes(x = type, fill = rating, col = rating)) + 
    geom_bar(position="dodge", alpha = 0.5) + 
    labs(x = 'Type', y = 'Count') +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave(file = 'plot/reviewer_type_bar.png')
